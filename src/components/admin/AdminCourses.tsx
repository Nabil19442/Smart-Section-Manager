...
export const AdminCourses: React.FC = () => {
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (modalMode === 'create') {
        const { error, fallbackUsed } = await insertWithCreatedByFallback('courses', {
          user_id: user?.id,
          created_by: user?.id,
        ...
        if (fallbackUsed) {
          ...
          showToast(
            'success',
            'Course added! (Note: Run schema migration in Supabase to link created_by to your admin account)'
          );
        } else {
